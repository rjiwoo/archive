...
def bfs(grid, start, target, wall):
    ...
    # # 포탄 사용 가능 여부
    # can_use_mega = int(my_allies['M'][3]) > 0
    # can_use_normal = int(my_allies['M'][2]) > 0
    ...
    while queue:
        ...
        for d, (dr, dc) in enumerate(DIRS):
            for dist in range(1, 4): # 사거리 3 이내
                nr, nc = r + dr*dist, c + dc*dist
                
                if not (0 <= nr < rows and 0 <= nc < cols): # 범위 밖이면 사격 중지
                    break
                if grid[nr][nc] == wall: # 벽이면 사격 중지
                    break
                if (nr, nc) == target: 
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            # 3단계부터 물 지형이 추가 되었기 때문에 '물'이면 이동하지 않는 코드 추가
            # 다음 이동 경로가 맵의 범위를 벗어나지 않고, 방문하지 않았고, '물'이 아니면 이동할 수 있는 다음 경로로 추가. 
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] != wall and (nr, nc) not in visited and grid[nr][nc] != 'W':
                # 나무 만나면 부셔
                if grid[nr][nc] == 'T':
                    visited.add((nr, nc))
